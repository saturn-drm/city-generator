...
#region Members
  ...
  /// </summary>
  private void RunScript(List<double> Density, double RemapIndex, ref object FAR)
  {
        // remap the input density into FAR
    // (0.5, 20)
    double max = Density.Max();
    double min = Density.Min();
    double factor = RemapIndex * 19.5 / (max - min);
    List<double> fars = new List<double>();

    for (int m = 0; m < Density.Count; m++) {
      double far = (Density[m] - min) * factor + 0.5;
      fars.Add(far);
    }
